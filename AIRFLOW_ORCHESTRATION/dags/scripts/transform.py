...
def transform_data(ti, **kwargs):
    """
    Transform the extracted data into Parquet format and store it as a file.
    """
    # Pull data from XCom
    ...
    output_file = "/mnt/c/Users/zaina/Documents/Core_Data_Engineering/capstone_project/AIRFLOW/dags/tmp/transformed_data.parquet"
...
input_file = "/mnt/c/Users/zaina/Documents/Core_Data_Engineering/capstone_project/AIRFLOW/dags/tmp/data.parquet"
...
def transform_data_s3(df):
    ...
    transformed_df = pd.DataFrame({
        "Country_Name": df["name"].apply(lambda x: x.get("common", "Unknown") if isinstance(x, dict) else "Unknown"),
        "Independence": df["independent"],
        "UN_Member": df["unMember"],
        "Start_of_Week": df["startOfWeek"],
        "Official_Name": df["name"].apply(lambda x: x.get("official", "Unknown") if isinstance(x, dict) else "Unknown"),
        "Common_Native_Name": df["translations"].apply(
            lambda x: x.get("nativeName", {}).get("common", "Unknown") if isinstance(x, dict) else "Unknown"
        ),
        ...
        if isinstance(x, dict) and x and list(x.values()) and isinstance(list(x.values())[0], dict) 
        else "No Currency Name"

        ),

        "Currency_Symbol": df["currencies"].apply(
            lambda x: list(x.values())[0].get("symbol", "No Currency Symbol") if isinstance(x, dict) and x and list(x.values()) and isinstance(list(x.values())[0], dict) else "No Currency Symbol"
        ),
        "Country_Code": df["idd"].apply(
            lambda x: f"{x.get('root', '')}{''.join(x.get('suffixes', []))}" if isinstance(x, dict) and x.get('root') else "No Country Code"
        ),
        ...
        "Sub_Region": df.get("subregion", pd.Series(["No Sub-Region"] * len(df))).apply(
            lambda x: x if pd.notna(x) else "No Sub-Region"
        ),

        "Languages": df["languages"].apply(
        lambda x: ", ".join([v for v in x.values() if isinstance(v, str)]) if isinstance(x, dict) else None
        ),
...
def run_transformation(load_function, table_name, schema_name, snowflake_conn_id):
    ...
    Args:
        load_function (callable): The function responsible for loading data to Snowflake.
